...
code_to_insert = '''
    ...
    try:
        ...
        try:
            ...
            # Extraer productos. Buscamos en la tabla de Productos del proveedor
            filas_productos = driver.find_elements(By.XPATH, "//div[contains(@class, 'ant-table-body')]//table/tbody/tr[contains(@class, 'ant-table-row')]")
            if not filas_productos:
                filas_productos = driver.find_elements(By.XPATH, "//table/tbody/tr")
            ...
            for fila in filas_productos:
                ...
                if len(celdas_prod) > 0:
                    ...
                    if texto and "No Data" not in texto and "Resultados" not in texto and "Result" not in texto:
                        # Si tiene icono con texto, ej: "rr\\nrustic rubber keyboard", tomamos la última parte
...
for file in files:
    ...
    # We will replace the previous navigation block with the new one
    # Regex to capture the block from the start of NAVEGACIÓN until just before the end of the try block, or we can just find the start and end tokens
    ...
    if start_token in content and end_token in content:
        ...
        print(f'Tokens not found in {file}, skipping or already updated.')
