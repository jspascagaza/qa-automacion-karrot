...
def generate_data(
    ...
    if int(attribute_count) < 1 or int(attribute_count) > 5:
        raise ValueError("attribute_count must be between 1 and 5")
    ...
    # Column headers and widths - Dynamic attributes
    base_columns = [
        ('Type*', 15),
        ('Name*', 30),
    ...
    attribute_columns = []
    attribute_names = ['color', 'material', 'tamaño', 'capacidad', 'conectividad'][:int(attribute_count)]
    for attr_name in attribute_names:
        attribute_columns.append((f'Attribute {attr_name.title()}', 15))
    ...
    columns = base_columns + attribute_columns + remaining_columns
    ...
    attribute_headers = []
    for attr_name in attribute_names:
        attribute_headers.append(f'Attribute {attr_name.title()}')
    
    final_headers_row_5 = [
        'Type*', 'Name*', 'Fotografía producto', 'Category*',
        'Grupo de Unidades', 'Unit*', 'Description*',
        'isPerishable*',
        *attribute_headers,  # Dynamic attributes
    ...
    connectivity = ["WiFi", "5G", "Bluetooth", "NFC", "USB-C", "Lightning"]
    ...
    for i in range(products_with_variants):
        ...
        for attr_idx, attr_name in enumerate(attribute_names):
            if attr_name == 'color':
                attribute_values.append(colors[attr_idx % len(colors)])
            elif attr_name == 'material':
                attribute_values.append(materials[attr_idx % len(materials)])
            elif attr_name == 'tamaño':
                attribute_values.append(sizes[attr_idx % len(sizes)])
            elif attr_name == 'capacidad':
                attribute_values.append(capacities[attr_idx % len(capacities)])
            elif attr_name == 'conectividad':
                attribute_values.append(connectivity[attr_idx % len(connectivity)])
            else:
                attribute_values.append(f"Valor{attr_idx+1}")

        for v_i in range(product_variant_count):
            # Create variant-specific attribute values
            variant_attributes = []
            for attr_idx, attr_value in enumerate(attribute_values):
                if attr_idx == 0:  # First attribute varies by variant
                    variant_attr_value = colors[(attr_idx + v_i) % len(colors)] if 'color' in attribute_names else f"Variante{v_i+1}"
                else:
                    variant_attr_value = attr_value
                variant_attributes.append(variant_attr_value)
            ...
            # Generate unique barcodes using UUID-like approach to avoid collisions
            barcode = f'{7700000000000 + (product_id * 1000) + (v_i * 100) + random.randint(0, 99)}'
            barcode2 = f'{7800000000000 + (product_id * 1000) + (v_i * 100) + random.randint(0, 99)}'
            barcode3 = f'{7900000000000 + (product_id * 1000) + (v_i * 100) + random.randint(0, 99)}'
            ...
            if inject_errors and random.random() < float(error_rate):
                ...
                if err == "missing_sku":
                    ...
                    if variant_attributes and random.random() < 0.5:
            ...
            raw_data = [
                ...
                description,
                is_perishable,
                *variant_attributes,  # Dynamic attributes
                sku,
                barcode,
                barcode2,
                barcode3,
            ...
            for idx, val in enumerate(raw_data):
                ...
                if idx == 4:
                    ...
                    fill = blue_fill
                elif idx >= 8 and idx < 8 + int(attribute_count):
                    fill = pink_fill
    ...
    for i in range(simple_products):
        ...
        # Generate unique barcodes for simple products
        barcode = f'{7700000000000 + (product_id * 1000) + random.randint(0, 99)}'
        barcode2 = f'{7800000000000 + (product_id * 1000) + random.randint(0, 99)}'
        barcode3 = f'{7900000000000 + (product_id * 1000) + random.randint(0, 99)}'
        ...
        raw_data = [
            ...
            description,
            is_perishable,
            *([''] * int(attribute_count)),  # Empty attributes for simple products
            sku,
            barcode,
            barcode2,
            barcode3,
...
if __name__ == "__main__":
    presets = {
        # Testing presets for multi-attribute issues
        "test_1_attr_ok": {
            "filename": "test_1_attr_ok.xlsx",
            "product_count": 10,
            "variants_per_product": 2,
            "products_with_variants": 10,
            "attribute_count": 1,
            "inject_errors": False,
            "error_rate": 0.0,
            "seed": 42,
        ...
        "test_3_attrs_issue": {
            "filename": "test_3_attrs_issue.xlsx",
            "product_count": 10,
            "variants_per_product": 2,
            "products_with_variants": 10,
            "attribute_count": 3,
        ...
        "test_5_attrs_issue": {
            "filename": "test_5_attrs_issue.xlsx",
            "product_count": 10,
            "variants_per_product": 2,
            "products_with_variants": 10,
            "attribute_count": 5,
        ...
        "stress_multi_attrs": {
            "filename": "stress_multi_attrs.xlsx",
            "product_count": 100,
            "variants_per_product": 2,
            "products_with_variants": 100,
            "attribute_count": 4,
            "inject_errors": False,
            "error_rate": 0.0,
            "seed": 42,
    ...
    parser.add_argument("--attribute-count", type=int, choices=[1, 2, 3, 4, 5])
